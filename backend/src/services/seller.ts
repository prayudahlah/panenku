...
export async function getPublicProfile(userId: number): Promise<ServiceResult<any>> {
    ...
    if (profile.status === 'suspended') {
        return {
            error: 'Penjual ini ditangguhkan',
            code: ERR.CAT_03,
            status: 403,
        };
    }
    ...
    return {
        data: {
            ...
            activeProductCount,
            // FIX: tambah createdAt sesuai kontrak response FSD-04.2
...
export async function getCatalogBySellerPublic(
    ...
    const message =
        catalogData.rows.length === 0 ? 'Penjual belum memiliki produk' : undefined;
